const maximumMinutes: any = function (grid) {
    ...
    function isValidCell(x: any, y: any): boolean {
        return x >= 0 && x < m && y >= 0 && y < n;
    }
    const fireDist: any = new Array(m);
    for (let i: number = 0; i < m; i++) {
        fireDist[i] = new Array(n).fill(Infinity);
    }
    const firePoints: any = [];
    for (let i: number = 0; i < m; i++) {
    ...
    while (left < right) {
        const middle: any = Math.floor((left + right + 1) / 2);
        if (canEscape(middle)) {
            left = middle;
        }
        else {
