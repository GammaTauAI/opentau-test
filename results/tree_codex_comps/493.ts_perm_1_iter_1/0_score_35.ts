var getAllElements: (T1: any, T2: any) => any = function (root1, root2) {
    const set1: Set<number> = new Set(), set2: Set<number> = new Set();
    traverse(root1, set1);
    traverse(root2, set2);
    const res: number[] = [];
    const it1: IterableIterator<number> = set1[Symbol.iterator]();
    const it2: IterableIterator<number> = set2[Symbol.iterator]();
