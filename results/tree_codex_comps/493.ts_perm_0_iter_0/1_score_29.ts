var getAllElements: (root1: any, root2: any) => number[] = function (root1, root2) {
    ...
    traverse(root2, set2);
    const res: number[] = [];
    const it1: Iterator<number, number, undefined> = set1[Symbol.iterator]();
    const it2: Iterator<number, number, undefined> = set2[Symbol.iterator]();
    ...
    return res;
    function traverse(node: any, set: Set<number>): Set<number> {
        if (node == null)
            return;
