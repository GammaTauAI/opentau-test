const isValid: (code: any) => any = function (code) {
    const STATES: { lt: number; // <
    tagOrData: number; // uppercase=tag, '!'=data
    tagName: number; // uppercase, '>'=end
    dataContent: number; // any, ']'=wait-for-end
    dataEnd: number; // any, ']'=end
    tagContent: number; } = {
        lt: 1,
        tagOrData: 2,
        tagName: 3,
    ...
    class Validator {
        ...
        validate(): boolean {
            ...
            switch (this.state) {
                ...
                case STATES.dataContent: {
                    ...
                    let end: any = this.findStrEnd(this.i, "]]>");
                    if (end != -1) {
                        // found end
                        this.i = end;
                        this.state = STATES.tagContent;
        ...
        isUpperCase(char: any): boolean {
            return /[A-Z]/.test(char);
        }
        findStrEnd(from: any, toFind?: string): any {
            let end: any = from + toFind.length;
            for (let i: any = 0; i < toFind.length; i++) {
                if (toFind[i] != this.str[i + from])
                    return -1;
            }
            return end;
        }
        findTagName(from: any): string {
            let tagName: string = "";
            for (let i: any = from; i < this.str.length; i++) {
