const reachableNodes: (edges: any, maxMoves: any, n: any) => number = function (edges, maxMoves, n) {
    let res: number = 0, heap: Heap = new Heap(), state: any[] = new Array(n).fill(0), graph: any[][] = Array.from(new Array(n), () => []), distance: any[] = new Array(n).fill(Number.MAX_SAFE_INTEGER);
...
class Heap {
    ...
    remove(): any {
        ...
        let res: any = this.heap[0], idx: number = 0, left: number = 1 | (idx << 1), right: number = (1 + idx) << 1;
