const minInterval: (intervals: any, queries: any) => any[] = function (intervals, queries) {
    const n: any = intervals.length;
    const m: any = queries.length;
    ...
    const res: any[] = Array(m).fill(0);
    let i: number = 0;
    for (const idx of sortedQueryIdx) {
        const query: any = queries[idx];
...
class BinaryHeap {
    ...
    remove(node: any): void {
        const length: any = this.content.length;
