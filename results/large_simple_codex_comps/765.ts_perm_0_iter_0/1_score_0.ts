const reachableNodes: (edges: Array<[number, number, number]>, maxMoves: number, n: number) => number = function (edges, maxMoves, n) {
    let res: number = 0, heap: Heap = new Heap(), state: number[] = new Array(n).fill(0), graph: Array<Array<[number, number]>> = Array.from(new Array(n), () => []), distance: number[] = new Array(n).fill(Number.MAX_SAFE_INTEGER);
...
class Heap {
    ...
    remove(): number[] {
        ...
        let res: number[] = this.heap[0], idx: number = 0, left: number = 1 | (idx << 1), right: number = (1 + idx) << 1;
