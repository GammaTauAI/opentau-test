const AutocompleteSystem: (sentences: string[], times: number[]) => void = function (sentences, times) {
    this.trie = new Trie();
    this.inputString = "";
    this.MAX_RESULTS = 3;
    for (let i = 0, len = times.length; i < len; i++) {
        this.trie.insert(sentences[i], times[i]);
    }
};
AutocompleteSystem.prototype.input = function (c) {
    if (c === "#") {
        this.trie.insert(this.inputString);
        this.inputString = "";
        return [];
    }
    this.inputString += c;
    const strings: any = this.trie.stringsStartingWith(this.inputString);
    const results: string[] = Object.keys(strings);
    results.sort((a, b) => {
        const aFreq: number = strings[a];
        const bFreq: number = strings[b];
        return aFreq !== bFreq ? bFreq - aFreq : a > b ? 1 : -1;
    });
    return results.slice(0, this.MAX_RESULTS);
};
// Using a Trie (Prefix tree).
const Trie: () => void = function () {
    this._trie = {};
};
Trie.prototype.insert = function (word, count = 1) {
    if (!word.length || count < 1) {
        return;
    }
    let curr: any = this._trie;
    for (let i = 0; i < word.length; i++) {
        const char: string = word[i];
        if (!curr.hasOwnProperty(char)) {
            curr[char] = {};
        }
        curr = curr[char];
    }
    if (!curr.hasOwnProperty("#")) {
        curr["#"] = 0;
    }
    curr["#"] += count;
};
// Time: O(n), where n is the number of different strings in the Trie.
// Space: O(1)
Trie.prototype.stringsStartingWith = function (prefix) {
    if (!prefix.length) {
        return false;
    }
    let curr: any = this._trie;
    for (let i = 0; i < prefix.length; i++) {
        const char: string = prefix[i];
        if (!curr.hasOwnProperty(char)) {
            return false;
        }
        curr = curr[char];
    }
    const results: any = {};
    function traverse(node: any, chars: string): void {
        if (!node) {
            return;
        }
        Object.keys(node).forEach((char) => {
            if (char === "#") {
                results[chars] = node[char];
                return;
            }
            traverse(node[char], chars + char);
        });
    }
    traverse(curr, prefix);
    return results;
};
