const reachableNodes: (edges: [number, number, number][], maxMoves: number, n: number) => number = function (edges, maxMoves, n) {
    let res: number = 0, heap: Heap = new Heap(), state: number[] = new Array(n).fill(0), graph: [number, number][][] = Array.from(new Array(n), () => []), distance: number[] = new Array(n).fill(Number.MAX_SAFE_INTEGER);
