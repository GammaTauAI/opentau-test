const eventualSafeNodes: (graph: number[][]) => number[] = function (graph) {
    const n: number = graph.length, memo: { [x: number]: boolean } = {}, visited: Set<number> = new Set(), res: number[] = [];
    for (let i = 0; i < n; i++) {
...
function dfs(graph: number[][], node: number, memo: { [x: number]: boolean }, visited: Set<number>): boolean {
    if (memo[node] != null)
        return memo[node];
