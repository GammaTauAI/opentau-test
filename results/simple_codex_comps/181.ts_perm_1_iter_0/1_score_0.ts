const findAllPeople: (n: number, meetings: [number, number, number][], firstPerson: number) => number[] = function (n, meetings, firstPerson) {
...
class UnionFind {
    arr: number[];
    constructor(n) {
        this.arr = Array(n).fill(null); // Note: the code auto-completion doesn't work, you have to fill with null
