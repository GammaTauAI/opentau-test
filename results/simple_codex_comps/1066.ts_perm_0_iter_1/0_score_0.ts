const maximumMinutes: (gridMatrix: number[][]) => number = function (grid) {
    const [m, n]: number[] = [grid.length, grid[0].length];
    const dir: number[][] = [
        [-1, 0],
        [1, 0],
        [0, -1],
    ...
    while (firePoints.length) {
        const [x0, y0]: number[] = firePoints.shift();
        for (const [dx, dy] of dir) {
            const [x1, y1]: number[] = [x0 + dx, y0 + dy];
    ...
    function canEscape(delay: number): boolean {
        ...
        while (queue.length) {
            ...
            for (let i = queue.length; i > 0; i--) {
                const [i0, j0]: number[] = queue.shift();
                visited[i0][j0] = true;
                for (const [di, dj] of dir) {
                    const [i1, j1]: number[] = [i0 + di, j0 + dj];
    ...
    let [left, right]: number[] = [-1, 1000000000];
    while (left < right) {
